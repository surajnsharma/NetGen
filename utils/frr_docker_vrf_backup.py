...
class FRRDockerManager:
    ...
    def _get_vrf_table(self, device_id: str) -> int:
        ...
        return self.vrf_table_base + (hash_int % 100)  # Use smaller range to avoid conflicts
        ...
        try:
            ...
            if result.returncode != 0:
                if "File exists" in result.stderr:
                    logger.info(f"[VRF] VRF {vrf_name} already exists")
                else:
                    logger.error(f"[VRF] Failed to create VRF {vrf_name}: {result.stderr}")
                    logger.error(f"[VRF] Command: ip link add {vrf_name} type vrf table {vrf_table}")
                    return False
...
def get_bgp_status(device_id: str) -> Dict:
    ...
    return frr_manager.get_bgp_status(device_id)

def get_bgp_neighbors(device_id: str) -> Dict:
    """Get BGP neighbors for a device"""
    return frr_manager.get_bgp_neighbors(device_id)
